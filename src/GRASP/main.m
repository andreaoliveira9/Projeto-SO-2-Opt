...
r = 3;              % tamanho da RCL
time = 5;          % tempo de execução por execução
...
allGRASPIterationsIterations = zeros(1, numRuns);
...
for i = 1:numRuns
    fprintf('\n--- Execução %d ---\n', i);
    [score, nodes, GRASPIterations, localSeachIterations, foundTime] = GRASP_SNS_Optimized(G, time, n, r, Cmax);
    ...
    allGRASPIterationsIterations(i) = GRASPIterations;
    ...
    fprintf('Média SP: %.4f | Max SP: %.4f | Tempo: %.2fs | Iterações Totais: %d | Iterações Local Search: %d\n', ...
            avgSP, maxSP, foundTime, GRASPIterations, localSeachIterations);
end
...
fprintf('Iterações totais: %.4f\n', mean(allGRASPIterationsIterations));
